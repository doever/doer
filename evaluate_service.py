...
class EvaluateConfig(ConfigMixin):
    ...
    def tips_map(self, Data):
        try:
            Tips_Map = {1: f"叫号:{Data['ticketNumber'] or ''}",
                        2: f"开始办理:{Data['ticketNumber'] or ''}",
                        3: f"办理结束:{Data['ticketNumber'] or ''}",
                        4: f"窗口：{Data['windowName'] or ''}--> 暂停服务",
                        5: f"窗口：{Data['windowName'] or ''}--> 恢复服务",
                        6: f"更新评价:{Data}",
                        7: f"窗口：{Data['windowName'] or ''} --> 用户登录",
                        8: f"窗口：{Data['windowName'] or ''} --> 用户退出",
                        }
        except KeyError:
            pass
        finally:
    ...
    def tips_map_re(self, Data):
        if Data['ticketNumber']:
            Tips_Map = {
                1: f"叫号:{Data['ticketNumber'] or ''}",
                2: f"开始办理:{Data['ticketNumber'] or ''}",
                3: f"办理结束:{Data['ticketNumber'] or ''}",
            }
        elif Data['windowName']:
            pass
